...
{	string sign, code, lfunc, stext, sdata="";
	...
	while(1){
		...
		tmp1 = GetStr("^[[:space:]]*⇓\\([[:space:]]*([0-9]+|[a-z])[[:space:]]*,[[:space:]]*([0-9]+|[a-z])[[:space:]]*\\)", code); 			//Редукция
		if(tmp1[0] != "$$$"){
			code = tmp1[2]; 
			if(tmp1[3][0] >= '0' && tmp1[3][0] <= '9') {
				stext += "  mov ebx, " + tmp1[3] + "\n";
			} 
			else {
				stext += "  mov ebx, [ebp +" + NToS(GetAddr(tmp1[3])) + "]\n"; 
			}
			if(tmp1[4][0] >= '0' && tmp1[4][0] <= '9') {
				stext += "  mov ecx, " + tmp1[4] + "\n";
			} 
			else {
				stext += "  mov ecx, [ebp +" + NToS(GetAddr(tmp1[4])) + "]\n"; 
			}
			
			
			if(tau == 0) {												//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				stext = stext + ReducTau();
			} 
			else {
				printf("tau != 0\n");
				stext = stext + ReducTauComp(metka);
			}
			continue;
		}
