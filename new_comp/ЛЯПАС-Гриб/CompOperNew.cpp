...
string ReducTau() {
	string text = "";
	text = text + "  cmp ebx, ecx\n  ja _errend\n  cmp ecx, 32\n  ja _errend\n  mov edi, 32\n  sub edi, ebx\n  mov edx, eax\n  shr eax, cl\n  mov ecx, ebx\n  shl eax, cl\n";
	text = text + "  xor esi, esi\n  dec esi\n  mov ecx, edi\n  shr esi, cl\n  and edx, esi\n  or eax, edx\n";
	return text;
}

/*
	mov ebx, <e1>	// е1 - указывает на первый вычеркнутый бит
	mov ecx, <e2>	// е2 - указывает на последний вычеркнутый бит
	cmp ebx, ecx
	ja _errend
	cmp ecx, 32
	ja _errend
...
string ReducTauComp(int& metka) {
	string text = "";
	text = text + "  mov eax, [ebp + 112]\n  shl eax, 5\n  cmp ebx, ecx\n  ja _errend\n  cmp ecx, eax\n  ja _errend\n";
	text = text + "  push ecx\n  mov ecx, ebx\n  shr ecx, 5\n  inc ecx\n  mov edx, ebp\n  sub edx, 64\n  mov esi, edx\n  push esi\n";
	text = text + "  mov edi, esi\n  shr eax, 3\n  sub edi, eax\n  push edi\n  std\n  rep movsd\n  mov eax, [esp+8]\n";
	text += RightShiftMod(metka);
...
/*
	...
	push ecx
	mov ecx, ebx		// количество баз для копирования
	shr ecx, 5
	inc ecx

	mov edx, ebp
	sub edx, 64
	mov esi, edx		// источник
	push esi

	mov edi, esi
	shr eax, 3
	sub edi, eax		// приемник
	push edi

	std
	rep movsd		// копируем tau 
